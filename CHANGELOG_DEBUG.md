...
## Web 手写识别界面 (2025-02-21)

### 功能概述

实现基于浏览器的实时手写数字识别界面，用户可通过网页手写数字并获得即时识别结果。

### 技术实现

| 组件 | 技术 |
|------|------|
| HTTP 服务 | cpp-httplib (单头文件库) |
| 前端 | HTML5 Canvas + JavaScript |
| API | RESTful JSON API |
| 图像处理 | 280×280 → 28×28 压缩，自动反色 |

### 文件结构

```
examples/web_demo.cpp    # Web服务主程序
include/microflow/httplib.h  # HTTP库
```
...
Response: {"digit": 7, "confidence": 0.9998, "scores": [...]}
```

### 功能特性

- ✅ 交互式画布 (支持鼠标和触摸)
- ✅ 实时图像预览 (显示发送给模型的28×28图像)
- ✅ 自动颜色反转 (白底黑字 → 黑底白字)
- ✅ 中心采样算法 (高质量压缩)
- ✅ CORS 支持 (跨域请求)
- ✅ 响应式设计 (移动端友好)
