...
| 指标 | 数值 | 说明 |
|------|------|------|
| MNIST推理延迟 | ~10ms | 单次推理平均时间 (优化后) |
| MNIST吞吐量 | ~100 inferences/sec | 每秒推理次数 (优化后) |
| 内存占用 | <10MB | 运行时内存（不含模型权重） |
| 模型大小 | <1MB | MNIST改进模型 |

**性能优化历史**:
- v2.0 (Transpose修复): 36 ms/inf, 28 inf/sec
- v3.0 (预转置权重): 10 ms/inf, 100 inf/sec (**3.5x加速**)
...
| 3.0 | 2025-02-21 | **转置权重优化，推理速度提升4.3倍** |
